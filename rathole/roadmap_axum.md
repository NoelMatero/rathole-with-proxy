# Technical Implementation Roadmap for `traffic-switch-proxy` (with axum-reverse-proxy)

This document provides a detailed, technical breakdown for implementing the `traffic-switch-proxy` by adapting the existing `rathole` codebase, with a focus on using `axum-reverse-proxy`.
...
## Phase 2: Implement Traffic Switching and Client Logic with `axum-reverse-proxy`
...
1.  **Integrate `axum-reverse-proxy` (`server.rs`):**
    *   Add the `axum-reverse-proxy` dependency to `Cargo.toml`, and remove `hyper-reverse-proxy`.
    *   Ensure you have a shared `hyper::Client` available in your `axum` state if needed by the proxy implementation.

2.  **Implement `axum` Fallback Handler (`server.rs`):**
    *   Add a `.fallback(proxy_handler)` to the `axum::Router`.
    *   The `proxy_handler` function will receive the `axum::extract::Request`.
...
3.  **Implement Traffic Switching Logic (`proxy_handler`):**
    ...
    *   **If tunnel is unhealthy:**
        1.  Use the `axum_reverse_proxy::ProxyHandler::proxy_service` to forward the request to the service's configured `fallback_url`.
        2.  This is a direct call that handles the `axum::Request` and returns an `axum::Response`, removing the need for manual conversions.
        3.  Return the response from the proxy function directly.
